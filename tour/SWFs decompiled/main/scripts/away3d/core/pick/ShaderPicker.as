package away3d.core.pick
{
   ...
   public class ShaderPicker implements IPicker
   {
      ...
      public function getViewCollision(x:Number, y:Number, view:View3D) : PickingCollisionVO
      {
         ...
         if(this._onlyMouseEnabled && (!this._hitEntity._ancestorsAllowMouseEnabled || !this._hitEntity.mouseEnabled))
         {
            return null;
         }
      ...
      private function getApproximatePosition(camera:Camera3D) : void
      {
         ...
         var scY:Number = NaN;
         var scZ:Number = NaN;
         var offsX:Number = NaN;
         var offsY:Number = NaN;
         var offsZ:Number = NaN;
      ...
      private function getPreciseDetails(camera:Camera3D) : void
      {
         ...
         var v2y:Number = NaN;
         var v2z:Number = NaN;
         var dot00:Number = NaN;
         var dot01:Number = NaN;
         var dot02:Number = NaN;
         var dot11:Number = NaN;
         var dot12:Number = NaN;
         var s:Number = NaN;
         var t:Number = NaN;
         var invDenom:Number = NaN;
         var u:Number = NaN;
         var v:Number = NaN;
         var ui1:uint = 0;
         var ui2:uint = 0;
         ...
         var s1z:Number = NaN;
         var nl:Number = NaN;
         var subGeom:ISubGeometry = SubMesh(this._hitRenderable).subGeometry;
         var indices:Vector.<uint> = subGeom.indexData;
         var vertices:Vector.<Number> = subGeom.vertexData;
         var len:int = int(indices.length);
         var i:uint = 0;
         var j:uint = 1;
         var k:uint = 2;
         ...
         var stride:int = int(subGeom.vertexStride);
         ...
         while(i < len)
         {
            ...
            if(!(x < x1 && x < x2 && x < x3 || y < y1 && y < y2 && y < y3 || z < z1 && z < z2 && z < z3 || x > x1 && x > x2 && x > x3 || y > y1 && y > y2 && y > y3 || z > z1 && z > z2 && z > z3))
            {
               ...
               if(s >= 0 && t >= 0 && s + t <= 1)
               {
                  ...
                  s = (dot11 * dot02 - dot01 * dot12) * invDenom;
                  t = (dot00 * dot12 - dot01 * dot02) * invDenom;
                  ui1 = uint(indices[i] << 1);
                  ui2 = uint(indices[j] << 1);
                  ui3 = uint(indices[k] << 1);
                  u = uvs[ui1];
                  v = uvs[ui1 + 1];
                  this._hitUV.x = u + t * (uvs[ui2] - u) + s * (uvs[ui3] - u);
                  this._hitUV.y = v + t * (uvs[ui2 + 1] - v) + s * (uvs[ui3 + 1] - v);
                  this._faceIndex = i;
                  this._subGeometryIndex = GeomUtil.getMeshSubMeshIndex(SubMesh(this._hitRenderable));
                  return;
               }
            }
            i += 3;
      ...
      public function dispose() : void
      {
         ...
         if(Boolean(this._triangleProgram3D))
         {
            this._triangleProgram3D.dispose();
         }
         if(Boolean(this._objectProgram3D))
         {
