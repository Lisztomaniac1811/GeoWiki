package away3d.bounds
{
   ...
   public class BoundingSphere extends BoundingVolumeBase
   {
      ...
      override public function isInFrustum(planes:Vector.<Plane3D>, numPlanes:int) : Boolean
      {
         ...
         {
            plane = planes[i];
            flippedExtentX = plane.a < 0 ? -this._radius : this._radius;
            flippedExtentY = plane.b < 0 ? -this._radius : this._radius;
            flippedExtentZ = plane.c < 0 ? -this._radius : this._radius;
            projDist = plane.a * (this._centerX + flippedExtentX) + plane.b * (this._centerY + flippedExtentY) + plane.c * (this._centerZ + flippedExtentZ) - plane.d;
            if(projDist < 0)
            {
               return false;
            }
      ...
      override public function fromSphere(center:Vector3D, radius:Number) : void
      {
         ...
         if(Boolean(_boundingRenderable))
         {
      ...
      {
         return new WireframeSphere(1,16,12,16777215,0.5);
      }
      ...
      override public function transformFrom(bounds:BoundingVolumeBase, matrix:Matrix3D) : void
      {
         ...
         _min.x = this._centerX - this._radius;
         _min.y = this._centerY - this._radius;
         _min.z = this._centerZ - this._radius;
         _max.x = this._centerX + this._radius;
         _max.y = this._centerY + this._radius;
         _max.z = this._centerZ + this._radius;
      }
   }
}
