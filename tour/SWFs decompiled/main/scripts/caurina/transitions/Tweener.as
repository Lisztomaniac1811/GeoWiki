package caurina.transitions
{
   ...
   public class Tweener
   {
      ...
      public static function addTween(p_scopes:Object = null, p_parameters:Object = null) : Boolean
      {
         ...
         var rTime:Number = isNaN(p_obj.time) ? 0 : Number(p_obj.time);
         var rDelay:Number = isNaN(p_obj.delay) ? 0 : Number(p_obj.delay);
         ...
         for(istr in p_obj)
         {
            ...
            {
               if(Boolean(_specialPropertySplitterList[istr]))
               {
                  ...
                  {
                     if(Boolean(_specialPropertySplitterList[splitProperties[i].name]))
                     {
         ...
         for(i = 0; i < rScopes.length; i++)
         {
            ...
            if(p_obj.overwrite == undefined ? autoOverwrite : Boolean(p_obj.overwrite))
            {
      ...
      public static function removeTweens(p_scope:Object, ... args) : Boolean
      {
         ...
         for(i = 0; i < args.length; i++)
         {
            ...
            {
               if(Boolean(_specialPropertySplitterList[args[i]]))
               {
         ...
         if(cTime >= tTweening.timeStart)
         {
            ...
            else
            {
               ...
               if(!tTweening.hasStarted)
               {
                  ...
                  {
                     if(Boolean(tTweening.properties[pName].isSpecialProperty))
                     {
               ...
               if(mustUpdate)
               {
                  for(pName in tTweening.properties)
                  {
                     ...
                     else if(Boolean(tProperty.hasModifier))
                     {
                     ...
                     if(Boolean(tProperty.isSpecialProperty))
                     {
      ...
      {
         return Boolean(p_tweening.useFrames) ? _currentTimeFrame : _currentTime;
      }
